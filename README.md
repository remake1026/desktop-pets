...
当前源码产品名称为 **line puppy**，版本为 1.1.1。Windows 构建文件名为 `line puppy-1.1.1-Win10-11-Setup.exe`，包含 x64 和 ia32 两种架构。

以下下载链接指向历史发布，尚未更新为本次改名后的构建：**[Windows v1.0.3](https://github.com/remake1026/desktop-pets/releases/download/v1.0.3/LineDog-1.0.3-Win10-11-Setup.exe)** · [版本说明](https://github.com/remake1026/desktop-pets/releases/tag/v1.0.3)。历史安装包内部仍可能显示旧名称。
...
4. 在“选定安装位置”页输入目录或点击“浏览”，可以选择其他磁盘，默认应用文件夹名为“line puppy”。
5. 在“安装选项”页设置 **创建桌面快捷方式** 和 **开机自动启动**。两个选项默认勾选，不需要时可取消。
6. 点击“安装”。完成后保留“运行line puppy”的勾选并点击“完成”，小狗就会出现在桌面上。以后可以从桌面快捷方式或开始菜单启动。

- 安装路径可以改为其他磁盘上的文件夹，支持中文路径。
- 默认安装文件夹和主程序名称为“line puppy”，卸载程序为“卸载line puppy.exe”。
- “创建桌面快捷方式”和“开机自动启动”均默认勾选，可在安装时取消；自启动会在登录 Windows 后启动小狗，卸载时清理对应设置。
- 默认仅为当前用户安装；也可以选择为所有用户安装，此时可能需要管理员授权。
- 卸载时，在 Windows 的应用列表中找到“line puppy”即可。
...
2. 没有 Homebrew 时，下载 `LineDog-1.1.0-Mac-arm64.dmg`，双击后将 **line puppy** 拖到 **Applications**。
3. 打开「访达 → 应用程序」，双击 **line puppy**。小狗会出现在桌面右下角附近，菜单栏右侧会出现图标。
...
- Homebrew 升级：`brew upgrade --cask line-dog`。卸载：`brew uninstall --cask line-dog`，或把「应用程序」里的「line puppy」移到废纸篓。
...
5. 安装完成后，line puppy会自动出现在桌面右下角附近。
...
- 鼠标右键点击小狗：显示关闭按钮
- 开机自动启动：Windows 可在托盘菜单勾选或取消“开机自启动”，也可在安装向导中设置。macOS 点击菜单栏图标，勾选或取消“登录时启动”。
- 卸载：在 Windows 的应用列表中卸载“line puppy”，或运行安装目录中的“卸载line puppy.exe”；卸载会清理该安装对应的自启动项。macOS 把应用程序里的“line puppy”移到废纸篓即可；若开启过登录启动，先在菜单栏取消勾选
...
- 查看任务栏或后台是否已经运行了一个实例
- macOS 可重新双击「应用程序」里的line puppy，或再执行一次 `brew reinstall --cask line-dog`
...
Mac 包尚未 Apple 公证。按住 Control 再点击应用选择“打开”，或执行 `xattr -cr "/Applications/line puppy.app"` 后再启动。
...
产物输出到项目根目录的 `dist/standard/`，包括 `line puppy.app`、`line puppy-1.1.1-Mac-arm64.dmg` 和对应 zip。构建机上会做 ad-hoc 签名，便于本地打开；分发给其他人时 macOS 仍可能提示未验证开发者。
...
这个项目是出于对「line puppy」的喜爱，以及学习和交流桌面应用开发的目的制作的。
