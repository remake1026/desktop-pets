...
- 锁定桌面：托盘菜单开启后桌宠始终置顶显示，鼠标点击直接穿透图层，不影响下方软件的拖动、缩放等操作；再次关闭即可恢复互动
...
“锁定桌面”模式下小狗是一个鼠标穿透的置顶层，点击会直接落到下方软件，因此无法再拖动或右键小狗。从系统托盘图标菜单里取消勾选“锁定桌面（鼠标穿透）”即可恢复互动。设置会自动记住，下次启动保持上次的状态。
