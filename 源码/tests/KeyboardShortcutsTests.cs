...
internal static class KeyboardShortcutsTests
{
    private static int Main()
    {
        ...
        keys.Handle(0x5A, 0); keys.Handle(0x5A, 1); // Plain Z does nothing.
        keys.Handle(0xA2, 0); keys.Handle(0x5A, 0); keys.Handle(0x5A, 0);
        keys.Handle(0xA2, 1); keys.Handle(0x5A, 1); // Releasing Ctrl finishes chord.
        keys.Handle(0xA3, 0); keys.Handle(0x5A, 0); keys.Handle(0x5A, 1); keys.Handle(0xA3, 1);
        ...
        var expected = "send-down,send-up,send-down,send-up,good-down,good-up,good-down,good-up,undo-down,undo-up,undo-down,undo-up,delete-down,delete-up,delete-down,delete-up,delete-down,delete-up,delete-down,delete-up";
        if (String.Join(",", events) != expected) throw new Exception("Shortcut transition mismatch: " + String.Join(",", events));
        Console.WriteLine("PASS: Enter, Ctrl+S, Ctrl+Z, Delete/Backspace, simultaneous holds, repeat suppression, key release");
        return 0;
    }
}
